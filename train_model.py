...
from data_generator import generate_gaussian, generate_cifar_dataset, generate_negative_dataset, generate_convex_dataset, get_mapping, PositiveDataSet, GaussianDataSet, NegativeDataSet, generate_adversarial, sample_dataset
...
cifar_train_set = datasets.CIFAR10(root=cifar_root, train=True, download=False)
mnist_train_set = datasets.MNIST(root=mnist_root, train=True, download=False)
fashion_train_set = datasets.FashionMNIST(root = fashion_root, train=True, download=False)
# ------------------------------------------------------------------------------------------------------------------
...
def train_model(new_model, optimizer, criterion, train_data_loader, test_data_loader, num_epochs, Writer, mnist_batch_size, convex_batch_size, 
                fashion_batch_size, gaussian_batch_size, cifar_batch_size, letter_batch_size, adversarial_train = False, 
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_data_loader):
            ...
            if convex_batch_size > 0:
                ...
                if gaussian_batch_size > 0:
                    gaussian_dataset = generate_gaussian(60000, mnist = (not use_cifar))
...
model_to_train = train_model(model_to_train, optimizer, criterion, train_loader, test_loader, num_epochs, SummaryWriter(),
                mnist_batch_size, convex_batch_size, fashion_batch_size, gaussian_batch_size, cifar_batch_size, letter_batch_size,
                 adversarial_train = adversarial_train, adversarial_batch_size = adversarial_batch_size , 
